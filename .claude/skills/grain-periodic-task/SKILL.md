...
```clojure
(ns app.my-service.core.periodic-tasks
  "Scheduled background jobs for my-service."
  (:require [app.my-service.core.read-models :as rm]
...
`es/tenants` (from `ai.obney.grain.event-store-v3.interface`) returns `{tenant-id {:tenant/last-event-id uuid-or-nil}}`. Use `(keys ...)` for just the ids, or read per-tenant metadata from the value.
...
```clojure
(ns app.my-service.interface
  (:require ...
            [app.my-service.core.periodic-tasks :as tasks]))
...
```clojure
;; In ns require:
[app.my-service.interface :as my-service]
