...
Most apps gate per-user data, so you must be signed in to verify them. The dev mailer (`app.email`
`logger-email`) only **logs** the verification token — it doesn't send mail — so create + verify a test
