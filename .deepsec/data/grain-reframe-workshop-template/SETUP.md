# Agent setup for `grain-reframe-workshop-template`

This is a Deepsec scanning workspace for `grain-reframe-workshop-template`
(target: `..`). Repository context is maintained in
`data/grain-reframe-workshop-template/INFO.md`.
...
2. **Review `data/grain-reframe-workshop-template/INFO.md`.** It's auto-injected into the AI
   prompt for every batch. Update it when the architecture or threat model changes.
...
```bash
bunx --no-install deepsec scan --project-id grain-reframe-workshop-template
bunx --no-install deepsec process --project-id grain-reframe-workshop-template --agent codex
```
